...
use lib "/home/apiwat/RNA_server/HeteroMirPred/ViennaRNA-1.8.5/Perl/blib/arch";
use lib "/home/apiwat/RNA_server/HeteroMirPred/HeteroMirPred/ViennaRNA-1.8.5/Perl/blib/lib";
	...
	# Fasta Third Line i.e. RNA secondary structure and MFE
		...
		if ($i == $numRandomSeqs) {
			if ($numSeqs > $statsID) {
				...
				printf (OUTFILE "%.2f\t%6.4f\t%6.4f\t%s\t%6.4f\t%s", $amfe[$numSeqs], $mean, $sd, $z, $p, $pz);
				($mean, $sd, $z, $p, $pz) = computeStats(\@cQ, $aQ[$numSeqs]);
				printf (OUTFILE "\t%6.2f\t%6.4f\t%6.4f\t%s\t%6.4f\t%s", $aQ[$numSeqs], $mean, $sd, $z, $p, $pz);
